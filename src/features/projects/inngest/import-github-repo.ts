...
export const importGithubRepo = inngest.createFunction(
	{
		...
		onFailure: async ({ event, step }) => {
			...
			await step.run('set-failed-status', async () => {
				await convex.mutation(api.requests.updateImportStatus, {
					internalKey,
					projectId,
					status: 'failed',
				});
	...
	async ({ event, step }) => {
		...
		await step.run('cleanup-project', async () => {
			await convex.mutation(api.requests.cleanup, {
				internalKey,
				projectId,
			});
		});
		...
		const folderIdMap = await step.run('create-folders', async () => {
			...
			for (const folder of folders) {
				...
				const folderId = await convex.mutation(api.requests.createFolder, {
					internalKey,
					projectId,
					name,
					parentId,
				...
				try {
					...
					if (isBinary) {
						const uploadUrl = await convex.mutation(
							api.requests.generateUploadUrl,
							{ internalKey },
						);
						...
						await convex.mutation(api.requests.createBinaryFile, {
							internalKey,
							projectId,
							name,
							storageId,
						...
						await convex.mutation(api.requests.createFile, {
							internalKey,
							projectId,
							name,
							content,
					...
					// biome-ignore lint/suspicious/noConsole: allow console in demo-project
